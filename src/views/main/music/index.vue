<template>
  <div
    class="flex items-center justify-between border-b border-[#e8e8e8] pb-[5px] px-5"
  >
    <div class="flex items-center gap-2">
      <Icon icon="lets-icons:sort-random-light" color="#c6c6c6" :width="20" />
      <Icon
        icon="tabler:player-track-prev-filled"
        color="#c6c6c6"
        :width="20"
      />
      <Icon icon="iconoir:play-solid" color="#c6c6c6" :width="30" />
      <Icon
        icon="tabler:player-track-next-filled"
        color="#c6c6c6"
        :width="20"
      />
      <Icon icon="cil:loop" color="#c6c6c6" :width="20" />
    </div>
    <div class="flex h-12">
      <div
  ...
  <div class="p-10">
    ...
    <div class="pt-11">
      ...
      <div class="flex gap-5 pt-3 cursor-pointer">
        <div v-for="item in musicInfoMetaList" class="w-1/6">
          <div
...
console.log("%c Line:97 🍧 audioFileKeys", "color:#ed9ec7", audioFileKeys);

const musicInfoMetaList = ref<any>([]);

const getMusicInfo = async (path: string) => {
  window.jsmediatags.read(`http://localhost:5173${path}`, {
    onSuccess: (tag: any) => {
      const meta: any = {
        path,
      };
      ...
      musicInfoMetaList.value.push(meta);
      console.log(
        "%c Line:126 🍅 musicInfoMetaList",
        "color:#fca650",
        musicInfoMetaList
      );
    },
...
onMounted(() => {
  for (let path of audioFileKeys) {
    console.log("%c Line:142 🍰 path", "color:#ffdd4d", path);
    getMusicInfo(path);
  }
});
</script>
