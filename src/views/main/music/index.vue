<template>
  <div class="flex items-center justify-between border-b border-[#e8e8e8] pb-[5px] px-5 text-[#c6c6c6]">
    <div class="flex items-center gap-2 cursor-pointer">
      <Icon icon="lets-icons:sort-random-light" :width="20" />
      <Icon icon="tabler:player-track-prev-filled" :width="20" />
      <Icon :icon="isPlaying ? 'iconoir:pause-solid': 'iconoir:play-solid'" :width="30" />
      ...
      <div
        ...
        <div class="hidden">
          <div class="absolute inset-0 flex items-center justify-center bg-black bg-opacity-30">
            <Icon icon="teenyicons:expand-alt-solid" color="white" :rotate="3" />
          </div>
        </div>
      </div>
      <div class="flex items-center justify-center w-96 border border-[#e5e5e5] border-l-0">
        <Icon icon="ic:baseline-apple" class="text-2xl" color="#707070" />
      </div>
    </div>
    <div class="flex items-center gap-8">
      <Icon icon="tabler:menu-2" color="#c6c6c6" width="20" height="20" />
      <button class="bg-[#c42926] rounded-md h-7 text-white px-3 text-[13px] flex items-center gap-1">
  ...
  <div class="p-10">
    ...
    <div class="pt-11">
      <h2 class="flex items-center gap-[2px] font-bold cursor-pointer">
        最新音乐
        <Icon icon="icon-park-outline:right" color="#c6c6c6" width="20" height="20" />
      </h2>
      <div class="flex gap-5 pt-3 cursor-pointer">
        <div v-for="item in audioInfoList" class="w-1/6">
          <div class="relative overflow-hidden rounded-lg shadow-xl music-cover aspect-square">
            <img :src="item.cover" alt="" srcset="" />
            <div class="absolute top-0 left-0 hidden w-full h-full bg-black bg-opacity-30">
              <div @click="play(item)"
...
const play = (audioInfo: AudioInfo) => {
  ...
  console.log("%c Line:158 🍪 currentAudioInfo.value", "color:#f5ce50", currentAudioInfo.value);
}
</script>
