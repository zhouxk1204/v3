<template>
  <div class="relative h-dvh flex flex-col">
    ...
    <div class="fixed bottom-[5px] px-3 w-full z-10">
      <MiniPlayer :coverUrl="currentMeta.coverUrl" :state="isPlaying ? 'play' : 'pause'"
        :title="currentMeta.title || ''" @toggle="onMiniPlayerToggle" @next="onMiniPlayerNext"></MiniPlayer>
    </div>
    <nav class="sticky top-0 bg-white z-10">
      <header class="flex items-center justify-center border-b-2 shadow-sm h-11 border-b-black/5">
        <a href="#">
    ...
    <div class="flex-1 overflow-auto">
      <div class="flex flex-col items-center gap-[18px] sm:gap-8 pb-0 pt-10 sm:p-10 sm:flex-row">
        <div
          class="w-[270px] aspect-square flex-none shadow-[0_10px_20px_0_rgba(0,0,0,0.1)] border-2 rounded-[10px] overflow-hidden">
          <img src="./favourite.webp" alt="cover" class="object-cover w-full h-full" />
        </div>
        <div class="flex flex-col items-center sm:items-start">
          <div>
            <h1 class="text-[26px] leading-[26px] text-black/85 font-bold flex items-center gap-1">
              <span>喜爱歌曲</span>
            ...
            <p class="text-xs font-bold text-center sm:text-left text-black/55 pt-[5px]">
              今日更新
            </p>
          </div>
      ...
      <div class="w-full px-4 pt-8 sm:px-10">
        ...
        <div @click="onClickListItem(index)"
          ...
          <div class="absolute -left-[15px] top-1/2 -translate-y-1/2">
            <span class="text-[#d60017]">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" width="9" height="9" class="svelte-17bwnrw">
                <path fill="currentColor"
                  d="M13.559 60.051c1.102.86 2.5.565 4.166-.645l14.218-10.455L46.19 59.406c1.666 1.21 3.037 1.505 4.166.645 1.102-.833 1.344-2.204.672-4.166l-5.618-16.718 14.353-10.32c1.666-1.183 2.338-2.42 1.908-3.764-.43-1.29-1.693-1.935-3.763-1.908l-17.605.108-5.348-16.8C34.308 4.496 33.34 3.5 31.944 3.5c-1.372 0-2.34.995-2.984 2.984L23.61 23.283l-17.605-.108c-2.07-.027-3.333.618-3.763 1.908-.457 1.344.242 2.58 1.909 3.763l14.352 10.321-5.617 16.718c-.672 1.962-.43 3.333.672 4.166Z">
                </path>
              </svg>
          ...
          <div class="flex-[4] flex items-center gap-3">
            <div class="relative">
              <img :src="music.coverUrl" alt="cover" height="40px" width="40px" class="rounded-[5px]" />
              <div v-if="selectIndex === index || playingIndex === index" @click="onClickCover($event, index)"
                class="absolute inset-0 bg-black/40 rounded-[5px] overflow-hidden flex items-center justify-center">
                <!-- loading -->
                <Icon v-if="isLoading && playingIndex === index" icon="line-md:loading-twotone-loop" width="18"
                  class="text-white" />

                <!-- pause -->
                <Icon v-else-if="isPlaying && playingIndex === index" icon="tabler:player-pause-filled" width="18"
                  class="text-white" />

                <!-- play -->
                <Icon v-else icon="tabler:player-play-filled" width="18" class="text-white" />
              </div>
            </div>
            <p class="text-black/85 font-[600] flex flex-col">
              <span class="text-[13px]" :class="selectIndex === index ? 'text-white' : 'text-black/85'">{{ music.title
                }}</span>
              <span class="sm:hidden text-[13px]" :class="selectIndex === index ? 'text-white' : 'text-black/55'">{{
      ...
      <section class="pb-12 pt-10 pl-10 bg-white">
        <p class="text-black/55 text-[13px]">
          {{ audioList.length }}&nbsp;首歌曲、{{ totalDuration.hours }}&nbsp;小时&nbsp;{{ totalDuration.minutes }}&nbsp;分钟
        </p>
      </section>
      <footer class="h-36 bg-[#fafafa] px-4 py-6">
        <span class="text-[10px] text-black/55">Powered By Zhouxk © {{ currentYear }}</span>
      </footer>
    </div>
  </div>
</template>
...
const totalDuration = computed(() => {
  const totalSeconds = audioList.value.reduce((sum, music) => sum + music.duration, 0);
